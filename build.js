#!/usr/bin/env node
/**
 * Build script for Page Duplicator WordPress plugin
 * 
 * This script creates a distribution-ready ZIP file of the plugin
 * with the correct WordPress directory structure.
 */
...
function createZipFile() {
  ...
  const zipFilePath = path.join(distDir, `${pluginSlug}-${pluginVersion}.zip`);
  ...
  // FIXED: This is the crucial change - archive just the plugin directory with its name
  // We're now adding the PLUGIN directory with its name, not the entire tempDir
...
async function build() {
  console.log(`🔄 Starting build process for ${pluginSlug} v${pluginVersion}...`);
